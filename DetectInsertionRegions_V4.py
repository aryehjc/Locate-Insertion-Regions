...
position_counts = {}
dir = '/name/of/my/alignments_fasta/directory'
...
for csv in csv_files:
    ...
    # Clean Header column
    df["Header"] = df["Header"].str.replace(r'^.*:|\($', '', regex=True)
    df["Header"] = df["Header"].str.split(r'\(').str[0]
    
    # Find start positions
    df['Start_Position'] = df.Header.str.split('-', expand=True)[0].astype(float)
    
    # Expand header to ranges
    df["Header"] = df["Header"].str.split(r'\s*-\s*').apply(lambda x: range(int(x[0]), int(x[-1]) + 1))
...
unique_counts_df.to_csv('Insertion_region_frequencies.csv', sep=',', index=False)
print(unique_counts_df)



# Before running script, later incooperate
#add 
#find /home/aryeh/SA_JC_2025/alignments_fasta -name '*.csv' \
#  -exec sh -c 'grep -q "[^[:space:]]" "$1" || echo "$1"' _ {} \;

# delete them (be sure!)
#find /home/aryeh/SA_JC_2025/alignments_fasta -name '*.csv' \
 # -exec sh -c 'grep -q "[^[:space:]]" "$1" || rm -v "$1"' _ {} \;
